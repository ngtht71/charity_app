...
contract CharityRegistry {
    ...
    modifier onlyValidCharity(uint charityId) {
        ...
        require(
            !compareStrings(charities[charityId].name, ""),
    ...
    // Ensure that the charity exists (but allow inactive)
    modifier onlyExistingCharity(uint charityId) {
        require(charityId <= charityIdCounter, "Invalid charity ID");
        require(
            !compareStrings(charities[charityId].name, ""),
            "Charity does not exist"
    ...
    function updateCharity(
        ...
        bool active,
        address payable wallet
    ) public onlyOwner onlyValidCharity(charityId) {
    ...
    // Allow the owner to update charity details even when the charity is currently inactive.
    // This helps update metadata or reactivate under an explicit call.
    function updateCharityAllowInactive(
        uint charityId,
        string memory name,
        string memory mission,
        string memory website,
        bool active,
        address payable wallet
    ) public onlyOwner onlyExistingCharity(charityId) {
        Charity storage charity = charities[charityId];
