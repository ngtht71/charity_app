...
const DATA_FILE = path.join(DATA_DIR, 'charity_meta.json');
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (id === undefined || meta === null) {
        return res.status(400).json({ error: 'Missing id or meta in request body' });
      }
      const all = await readAll();
      all[id] = meta;
      await writeAll(all);
      return res.status(200).json({ ok: true, id, meta });
    }

    return res.status(405).json({ error: 'Method not allowed' });
  } catch (err: any) {
    console.error('charity-meta API error', err);
